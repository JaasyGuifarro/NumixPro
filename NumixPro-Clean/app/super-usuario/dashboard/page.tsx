...
setVendors(vendors.map((v) => (v.id === id ? { ...v, showPassword: !v.showPassword } : v)))
  ...
  const handleMigrateData = async () => {
    ...
    try {
      ...
      if (success) {
        alert("Datos migrados correctamente a Supabase")
      } else {
        alert("Error al migrar datos a Supabase")
      }
    } catch (error) {
      ...
      <div className="sticky top-0 z-50 bg-black/90 backdrop-blur-sm border-b border-white/10">
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => handleSecureLogout(router)} className="bg-[#FF6B6B] hover:bg-[#FF5252] text-white">
                    ...
                    <TableRow key={vendor.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center">
                          <span className="mr-2">{vendor.password ? vendor.password : "••••••"}</span>
                          <Button
                            size="sm"
                            variant="ghost"
                          ...
                          >
                            {vendor.password ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                          </Button>
                        </div>
                      </TableCell>
